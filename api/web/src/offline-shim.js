...
(function () {
  ...
  const safeParse = (value) => {
    ...
    try {
      ...
      console.warn('[offline-shim] unable to parse KV cache', error);
      return {};
    }
  };
  ...
  const persistKv = () => {
    ...
    try {
      ...
      console.warn('[offline-shim] cannot persist KV cache', error);
    }
  };
  ...
  window.fetch = function (resource, init = {}) {
    ...
    return Promise.reject(new Error('Fetch unavailable in offline mode.'));
  };
  ...
  function handleSparkRequest(url, method, init) {
    ...
    if (path.startsWith('/kv/')) {
      ...
      if (method === 'POST') {
        const payload = typeof init.body === 'string' ? init.body : JSON.stringify(init.body || null);
    ...
    if (path === '/loaded' || path === '/status') {
      return jsonResponse({ status: 'offline', reason: reasonLabel, timestamp: new Date().toISOString(), source: 'offline-shim' });
    }
