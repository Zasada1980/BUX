...
async def start_link_telegram(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        text = (
            ...
            f"Введите Telegram username рабочего:\n"
...
async def receive_telegram_username(message: Message, state: FSMContext):
    """Receive and validate Telegram username"""
    ...
    # Try to find Telegram user by username
    db = next(get_db())
    try:
        # Check if username is already linked to another user
        existing = db.query(crud_users.User).filter(
            crud_users.User.telegram_username == username,
            crud_users.User.id != user_id
        ).first()
        
        if existing:
            await message.reply(
                f"⚠️ Username @{username} уже привязан к:\n"
                f"• {existing.name or 'ID ' + str(existing.id)}\n\n"
                f"Введите другой username или отмените:"
            )
            return
        
        # Update user with Telegram username
        from api.models_users import UserUpdate
        updated_user = crud_users.update_user(
            db, 
            user_id, 
            UserUpdate(telegram_username=username)
        )
        ...
        if updated_user:
            ...
            try:
                ...
                # Пытаемся отправить сообщение по @username
